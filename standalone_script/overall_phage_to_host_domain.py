...
def predict_on_crispr(file,outdir,min_mis_crispr=2,min_cov_crispr=70):
	...
	if not os.path.exists(python_path):
		python_path = "python"
	command = python_path+" "+script+" --input "+file+" --output "+outdir+" --min_mis_crispr "+str(min_mis_crispr)+" --min_cov_crispr "+str(min_cov_crispr)
	os.system(command)
...
def predict_on_virhostmatcher(input_dir,phage_hit_hosts_now,outdir):
	script = os.path.join(root_path,'script','PHIS_virhostmatcher.py')
	python_path = os.path.join(root_path,'software/python/miniconda3/bin','python')
	if not os.path.exists(python_path):
		python_path = "python"
	command = python_path+" "+script+" --input_dir "+input_dir+" --output "+outdir+" --out_dict "+phage_hit_hosts_now
	with open(os.path.join(outdir,'script'),'w') as f:
		f.write(command)
	os.system(command)

def predict_on_wish(input_dir,phage_hit_hosts_now,outdir):
	script = os.path.join(root_path,'script',"PHIS_wish.py")
	python_path = os.path.join(root_path,'software/python/miniconda3/bin','python')
	if not os.path.exists(python_path):
		python_path = "python"
	command = python_path+" "+script+" --input_dir "+input_dir+" --output "+outdir+" --out_dict "+phage_hit_hosts_now
...
def predict_on_codon(file,phage_hit_hosts_now,outdir):
	script = os.path.join(root_path,'script',"PHIS_codon.py")
	python_path = os.path.join(root_path,'software/python/miniconda3/bin','python')
	if not os.path.exists(python_path):
		python_path = "python"
	command = python_path+" "+script+" --input_file "+file+" --output "+outdir+" --type "+type+" --out_dict "+phage_hit_hosts_now
...
def get_length(file,outdir,new_fna_file):
	#fasta or multi-fasta
...
if __name__=='__main__':
	...
	phages_length_dict = get_length(input_file,strain_info_dir,new_fna_file)
	if type=='fasta':
