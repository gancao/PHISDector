...
def blastn_long(file,outfile,database,format,evalue):
	num_threads = 30
	format = 6
	script = "blastn -db "+database+" -query "+file+" -outfmt "+str(format)+" -evalue "+str(evalue)+" -out "+outfile+" -num_threads "+str(num_threads)+" -word_size 11 -reward 1 -penalty -2 -gapopen 0 -gapextend 0 -perc_identity 70 -max_target_seqs 100000"
	os.system(script)
...
def blastn_file(query_file,subject_file,outfile):
	...
	script = "blastn -subject "+subject_file+" -query "+query_file+" -outfmt "+str(format)+" -evalue "+str(evalue)+" -out "+outfile
	os.system(script)
